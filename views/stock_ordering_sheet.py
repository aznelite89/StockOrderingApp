...
if all([po_prod_file, po_sales_file, warehouse_file, original_file, plist_file, special_file]):
    ...
    # Process special order data to get customer allocation information
    ...
    # For products with allocations but no special orders, create empty customer allocation
    alloc_without_special = alloc_df[~alloc_df["Product Code"].isin(special_ProductCode) & (alloc_df["Allocated"] > 0)].copy()
    if not alloc_without_special.empty:
        ...
        customer_allocations = pd.concat([customer_allocations, alloc_without_special[["Product Code", "Customer Allocations", "Allocated Qty"]]], ignore_index=True)
