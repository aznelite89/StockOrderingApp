...
def _build_sample_xlsx() -> bytes:
    """Return the empty 6-tab sample workbook. Cached so we don't rebuild on every rerun."""
    ...
    with pd.ExcelWriter(sample_output, engine='xlsxwriter') as writer:
        ...
        product_data = pd.DataFrame({
            'Product Code': [], 'Supplier Code': [], 'Supplier Product Code': [],
            'Bin Location': [], 'Supplier Product Description': [], 'Product Group': [],
            'On Purchase': [], 'Allocated': [], 'Obsolete': [], 'Base Unit': [],
        })
        ...
        sales_data = pd.DataFrame({
            'Product Code': [], 'Product Description': [], 'Pref Supplier': [], 'Warehouse': [],
        ...
        # Tab 3: Warehouse Data
        warehouse_data = pd.DataFrame({
            '*Product Code': [], 'Warehouse Code': [], '*SOH': [],
            '*AverageLandCost': [], 'LastCost': [],
        })
        ...
        # Tab 5: Transaction Detail
        transaction_data = pd.DataFrame({
            'Transaction Date': [], 'Transaction Ref': [], 'Warehouse': [], 'Transaction Type': [],
            'Product Code': [], 'Product Description': [], 'Value': [], 'Quantity': [], 'Running Total': [],
        })
        ...
        special_data = pd.DataFrame({
            'Order No.': [], 'Order Date': [], 'Required Date': [], 'Completed Date': [],
            'Warehouse': [], 'Customer': [], 'Customer Type': [], 'Product': [],
            'Product Code': [], 'Product Group': [], 'Status': [], 'Quantity': [], 'Sub Total': [],
        })
    ...
    return sample_output.getvalue()


@st.cache_data(show_spinner=False)
def _load_and_process(po_prod_bytes: bytes, po_sales_bytes: bytes, warehouse_bytes: bytes,
                      plist_bytes: bytes, original_bytes: bytes, special_bytes: bytes,
                      coverage_weeks: int):
    """Load the 6 CSVs and produce the final ordered dataframe + special-order product codes.
    ...
    month_cols = sales_df.columns[13:16]
    ...
    customer_allocations = special_df.groupby("Product Code").agg({
        "Customer": lambda x: ", ".join(x.dropna().unique()),
        "Quantity": "sum",
    }).reset_index()
    customer_allocations = customer_allocations.rename(columns={
        "Customer": "Customer Allocations",
        "Quantity": "Allocated Qty",
    })

    alloc_without_special = alloc_df[
        ~alloc_df["Product Code"].isin(special_ProductCode) & (alloc_df["Allocated"] > 0)
    ].copy()
    if not alloc_without_special.empty:
        alloc_without_special["Customer Allocations"] = "Unknown Customer"
        alloc_without_special["Allocated Qty"] = alloc_without_special["Allocated"]
        customer_allocations = pd.concat(
            [customer_allocations, alloc_without_special[["Product Code", "Customer Allocations", "Allocated Qty"]]],
            ignore_index=True,
        )
    ...
    df = (static_df
        ...
        .merge(customer_allocations, on="Product Code", how="left"))
    ...
    final_cols = [
        ...
        "On Hand", "On Purchase Order", "Allocated", "Customer Allocations", "Available Stock",
    ...
    # --- Bin grouping (vectorised) ---
    # Same intent as the original nested loop: bins are ordered by the highest
    # `Need To Order` they contain, and rows inside each bin stay together
    # sorted by `Need To Order` desc. We assign each bin a rank equal to the
    # row index of its first occurrence in a need-desc sort, then sort the
    # whole frame on (bin_rank, need_desc). O(n log n) instead of O(n²).
    df["Bin Location"] = df["Bin Location"].fillna("UNKNOWN").astype(str)
    df_sorted = df.sort_values(by="Need To Order", ascending=False, kind="mergesort").reset_index(drop=True)
    bin_rank = df_sorted.drop_duplicates(subset="Bin Location", keep="first")[["Bin Location"]].copy()
    bin_rank["__bin_rank"] = range(len(bin_rank))
    df = df_sorted.merge(bin_rank, on="Bin Location", how="left")
    df = df.sort_values(by=["__bin_rank", "Need To Order"], ascending=[True, False], kind="mergesort")
    df = df.drop(columns=["__bin_rank"]).reset_index(drop=True)
...
def _build_excel(df: pd.DataFrame, supplier_input: str, special_ProductCode: tuple) -> bytes:
    """Build the multi-sheet xlsx output. Returns the raw bytes."""
    special_set = set(special_ProductCode)
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        if supplier_input.strip():
            ...
            for supplier_code in supplier_codes:
                ...
                start_row = 5
                headers = ['Line no.', 'Product Code', 'Supplier Product Code', 'Product Description',
                           'Weight', 'Qty (pcs)', 'Total Weight', 'Notes']
                for col_num, header in enumerate(headers):
                    worksheet.write(start_row, col_num, header, header_format)

                # 1000 placeholder rows for the macro to fill in
                for i in range(1, 1001):
                    excel_row = start_row + i
                    for col in range(8):
                        worksheet.write(excel_row, col, "")
...
supplier_input = st.text_input(
    "Enter Supplier Code(s) (comma separated, case insensitive):",
    value="",
    help="Enter one or multiple supplier codes separated by commas, e.g., ABC123, DEF456, GHI789",
)
...
if all([po_prod_file, po_sales_file, warehouse_file, original_file, plist_file, special_file]):
    with st.spinner("Loading and processing data — first run can take 30–60s on slow hosts..."):
        df, special_ProductCode = _load_and_process(
            po_prod_file.getvalue(),
            po_sales_file.getvalue(),
            warehouse_file.getvalue(),
            plist_file.getvalue(),
            original_file.getvalue(),
            special_file.getvalue(),
            int(coverage_weeks),
        )
    ...
    # --- Excel build (gated behind a button) ---
    st.divider()
    st.subheader("Download Excel")

    # Cache key invalidates if supplier list, coverage, or row count changes.
    excel_key = f"excel_v1_{hash((supplier_input.strip().upper(), int(coverage_weeks), len(df)))}"

    if st.button("📊 Build Excel for download", type="primary"):
        for k in list(st.session_state.keys()):
            if str(k).startswith("excel_v1_") and k != excel_key:
                del st.session_state[k]
        with st.spinner("Building Excel file..."):
            st.session_state[excel_key] = _build_excel(df, supplier_input, special_ProductCode)
    ...
    if excel_key in st.session_state:
        ...
        st.info("Click **Build Excel for download** above to generate the file. The preview above already reflects the current data.")
