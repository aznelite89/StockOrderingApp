...
- `<SUPPLIER>` — per-supplier order form with a green "PROCESS ORDER" button and 1000 placeholder rows. The button is wired to a VBA macro (`ProcessOrderMacro`) that pulls from the `<SUPPLIER>_ALL` tab using **hard-coded column letters** (`A`=Product Code, `B`=Supplier Code, `C`=Supplier Product Code, `D`=Supplier Description, `E`=Weight, `U`=Searay Order qty, `V`=Comments). If you reorder `final_cols` near `app.py:303`, you must update the VBA letter references near `app.py:545` to match — there is no programmatic link between them.
