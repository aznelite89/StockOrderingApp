...
4. **Product List** — no skip. Only `Weight` is consumed.
...
- `UNL_Order_Sheet` — full sheet; Product Code cells are hyperlinks to `searay.net.au/search?q=<code>`; rows whose Product Code appears in the Special Order export are yellow-highlighted.
- `<SUPPLIER>` — per-supplier order form with a green "PROCESS ORDER" button and 1000 placeholder rows. The button is wired to a VBA macro (`ProcessOrderMacro`) that pulls from the `<SUPPLIER>_ALL` tab using **hard-coded column letters** (`A`=Product Code, `B`=Supplier Code, `C`=Supplier Product Code, `D`=Supplier Description, `E`=Weight, `T`=Need To Order (the order qty → "Qty (pcs)"), `W`=Comments → "Notes"). These letters are positions in `final_cols`/`_ALL`, NOT the column names: `T`=Need To Order, `U`=Base Unit, `V`=Searay Order (blank placeholder), `W`=Comments. If you reorder `final_cols` (in `views/stock_ordering_sheet.py`, ~line 218) you MUST update the VBA letter references in the same file (~line 380) to match — there is no programmatic link between them. (Historical bug, fixed 2026-06-04: the macro read `U`/`V`, so "Qty (pcs)" showed "Each".)
...
- The version string is embedded twice (page title and H1, both `v4.8`); bump both together if releasing a new version.
